...
async function render(image0, image1) {
  ...
  function drawScense() {
    ...
    for (let i = 0; i < 10; i++) {
      ...
      model = model.translate(cubePositions[i][0], cubePositions[i][1], cubePositions[i][2]);
      let angle = 20 * i;
      if (i % 3 === 0) {
        angle = new Date().getTime() * 25;
      }
      model = model.rotate(angle, 1, 0.3, 0.5);
      ourShader.setMat4('model', model.elements);
