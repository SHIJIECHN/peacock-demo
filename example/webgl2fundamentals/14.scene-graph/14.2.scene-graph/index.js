...
function main() {
  ...
  const moonOribtNode = new Node();

  // 月球离太阳 20 个单位距离
  moonOribtNode.localMatrix = m4.translation(30, 0, 0);

  // 定义所有的节点
  const sunNode = new Node();
  sunNode.localMatrix = m4.scaling(5, 5, 5); // 太阳在中心
  sunNode.drawInfo = {
    uniforms: {
  ...
  moonOribtNode.setParent(earthOrbitNode);
  moonNode.setParent(moonOribtNode);
  ...
  function drawScene(time) {
    ...
    m4.multiply(m4.yRotation(0.01), moonOribtNode.localMatrix, moonOribtNode.localMatrix);
