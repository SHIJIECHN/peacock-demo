...
import { vsColor, fsColor } from './2.3.basic_lighting'
import { vsCube, fsCube } from './2.3.light_cube'
...
export default async function main(gl: WebGL2RenderingContext, canvas: HTMLCanvasElement) {
  ...
  const drawScene = (time: number) => {
    ...
    // 模拟光源移动
    lightPos[0] = 1 + Math.sin(time / 1000) * 2;
    lightPos[1] = Math.sin(time / 1000 / 2);
    lightingShader.setVec3('lightPos', lightPos);

    lightingShader.setVec3('viewPos', camera.position); // 摄像机的位置
