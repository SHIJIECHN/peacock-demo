...
import lightCubeVs from './3.1.light_cube.vs'
import lightCubeFs from './3.1.light_cube.fs'
import materialsVs from './3.1.materials.vs'
import materialsFs from './3.1.materials.fs'
...
export default async function main(gl: WebGL2RenderingContext, canvas: HTMLCanvasElement) {
  ...
  const drawScene = (time: number) => {
    ...
    let lightColor: vec3 = [Math.sin(time * 2), Math.sin(time * 0.7), Math.sin(time * 1.3)]; // 光颜色随时间变换

    let diffuseColor = vec3.multiply(vec3.create(), lightColor, [0.5, 0.5, 0.5]); // 降低影响
    let ambientColor = vec3.multiply(vec3.create(), diffuseColor, [0.2, 0.2, 0.2]); // 很低的影响
    lightingShader.setVec3('light.ambient', ambientColor); // 环境光照通常设置为一个比较低的强度
    lightingShader.setVec3('light.diffuse', diffuseColor); // 光源的漫反射分量通常设置为希望光所具有的那个颜色。通常是比较亮的白色
    ...
    lightingShader.setVec3("material.ambient", [1, 0.5, 0.31]);
    lightingShader.setVec3("material.diffuse", [1, 0.5, 0.31]);
