...
export class ObjReader {
  ...
  private parseMtl(data: string) {
    ...
    let callback = function (value: string, index: number, array: string[]) {
      ...
      if (tokens[0] === 'newmtl') {
        let tmpGroup: MtlGroup = new MtlGroup();
        ret.mtlGroups.set(tokens[1], tmpGroup);
        groupIndex = tokens[1];
      } else if (tokens[0] === 'Ns') {
        // @ts-ignore
        ret.mtlGroups.get(groupIndex).Ns = Number.parseFloat(tokens[1]);
      } else if (tokens[0] === 'Ka') {
        let tmpGroup: MtlGroup | undefined = ret.mtlGroups.get(groupIndex);
        if (!tmpGroup) {
          return;
        }
        ...
        tmpGroup.Ni = Number.parseFloat(tokens[1]);
      }
      else if (tokens[0] === 'd') {
        let tmpGroup: MtlGroup | undefined = ret.mtlGroups.get(groupIndex);
        if (!tmpGroup) {
        ...
        tmpGroup.d = Number.parseFloat(tokens[1]);
      }
        ...
        tmpGroup.illum = Number.parseInt(tokens[1]);
      }
